...
{
    public class PresseroOrderItem
    {
        ...
        public static bool IsInventoryItem(PresseroOrderItem item)
        {
            // Future proofing this, I could get away with just checking product 
            // attribute values at the time of implementing this.
            var index = Array.FindIndex(
                item.GetAttributeNamesAsArray(),
                x => !string.IsNullOrEmpty(x) && x.Equals("Billing Category",
                              StringComparison.InvariantCultureIgnoreCase));

            if (index != -1)
            {
                var billingCat = item.GetAttributeValuesAsArray()[index];
                if (!string.IsNullOrEmpty(billingCat) &&
